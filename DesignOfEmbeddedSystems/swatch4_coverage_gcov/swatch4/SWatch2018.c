...
void manageTimeSet_SETHOURS(struct FSM *fsm, enum Signal signal, uint8_T *SWatch2018_Y_hours,
	...
	if(!fsm->isEntryDone) {
		...
		fsm->prevState = functionsState; // this completes transition
		fsm->isEntryDone = 1;
		return;
	}
...
void manageTimeCountStopwatch_COUNT(struct FSM *fsm, enum Signal signal, uint8_T *SWatch2018_Y_hours,
	...
	switch(signal) {
	case SIG_SWATCHMODE:
		//printf2UART("manageTimeCountStopwatch_COUNT received a SIG_SWATCHMODE\r\n");
		break;
	case SIG_ALARMSETMODE:
	case SIG_TIMESETMODE:
	case SIG_TIMEMODE:
		//printf2UART("manageTimeCountStopwatch_COUNT received a SIG_{Å,T,TS}\r\n");
		tranFSM(fsm, TIMECOUNTSTOPWATCH_BEGIN);
		break;
	default:
		break;
...
void manageTIMEDISPLAY(struct FSM *fsm, enum Signal signal, uint8_T *SWatch2018_Y_hours,
    ...
    switch(signal) {
        case SIG_TIMEMODE:
        	//printf2UART("manageTIMEDISPLAY(). Received SIG_TIMEMODE\r\n");
        ...
        case SIG_ALARMSETMODE:
            ...
            break;
        case SIG_SWATCHMODE:
        	//printf2UART("manageTIMEDISPLAY(). Received SIG_SWATCHMODE\r\n");
...
void initFSM(struct FSM *fsm, enum Subcomponent sc) {
    ...
    case SC_TIMECOUNTSTOPWATCH:
		...
		fsm->prevState = TIMECOUNTSTOPWATCH_BEGIN;
    	break;
    case SC_TIMESET:
    	fsm->curState = TIMESET_SETHOURS;
    	fsm->prevState = TIMESET_SETHOURS;
    	break;
    case SC_ALARMSET:
    	fsm->curState = ALARMSET_INIT;
    	fsm->prevState = ALARMSET_INIT;
    	break;
    case SC_ALARMMANAGEMENT:
    	fsm->curState = ALARMMANAGEMENT_SILENT;
    	fsm->prevState = ALARMMANAGEMENT_SILENT;
    	break;
    default:
    	break;
    }
